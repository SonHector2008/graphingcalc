...
def plot_custom_regression_analysis(df, degree):
    ...
    all_coefficients = np.append(coefficients[1:], intercept)
    polynomial_equation = format_polynomial_equation(all_coefficients, degree)
    
    ax.scatter(df['Day'], df['Price'], alpha=0.6, color='green', s=30, label='Actual Data')
    ax.plot(df['Day'], y_pred_poly, color='red', linewidth=3, 
            label=f'Polynomial Regression (Degree {degree})\nR² = {r2_poly:.4f}')
    ax.set_xlabel('Days (from March 2024)', fontsize=12)
    ax.set_ylabel('Bitcoin Price ($)', fontsize=12)
    ax.set_title(f'Bitcoin Price - Polynomial Regression (Degree {degree})', fontsize=14, fontweight='bold')
